...
class PreProcessLandmarksWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    self.DegradationInterface.enabled = False   # Wait for valid markups node(s) selection before enabling repair
    ...
    self.RepairInterface.enabled = False    # Wait for valid markups node selection before enabling repair
    ...
    self.kmWindowSizeSlider.setToolTip("Sets the number of points used in each iteration of k-means for right-left categorization")
    ...
    self.PolyFitDegreeSlider.setToolTip("Sets the number degrees of the polynomials fit to right and left R-L, A-P coordinates, as well as their curve-wise point frequencies")
    ...
    self.SpecificitySlider.setToolTip("Sets a scalar multiplier used to compare interval measures to curve statistics")
    self.SpecificitySlider.enabled = True
    self.SpecificitySlider.minimum = -10
    self.SpecificitySlider.maximum = 10
    self.SpecificitySlider.value = 2
    self.RepairConfigInterfaceLayout.addWidget(qt.QLabel("AutoGen specificity"), 3, 0, 1, 1)
    self.RepairConfigInterfaceLayout.addWidget(self.SpecificitySlider, 3, 1, 1, 3)
  ...
  def OnSelectedNodeChanged(self):
    ...
    if self.ModifyAllNodesCheckBox.isChecked():
      ...
      self.RepairInterface.collapsed = (self.SingleNodeSelector.currentNode() == None) or self.RepairInterface.collapsed
      self.RepairInterface.enabled = (self.SingleNodeSelector.currentNode() != None)
    
      self.DegradationInterface.collapsed = self.SingleNodeSelector.currentNode() == None or self.DegradationInterface.collapsed
      self.DegradationInterface.enabled = self.SingleNodeSelector.currentNode() != None
  ...
  def OnCategorizeLeftRight(self):
    ...
    if self.logic == None:
      self.logic = PreProcessLandmarksLogic(self.SingleNodeSelector.currentNode(), self.kmWindowSize, self.PolynomialDegree, self.SpecificitySlider.value)
      self.SpecificitySlider.enabled = True
